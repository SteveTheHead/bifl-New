  ...
  const fetchGuide = async () => {
    try {
      ...
      if (data.guide) {
        ...
        setSlug(guide.slug)
        setFeaturedImageUrl(guide.featured_image_url || '')
        setMetaTitle(guide.meta_title || '')
        setMetaDescription(guide.meta_description || '')
        setIntroContent(guide.intro_content || '')
        setCurationId(guide.curation_id || '')
        setIsPublished(guide.is_published)
        setWasPublished(guide.is_published)
        setBuyingCriteria(guide.buying_criteria || [])
        setFaqs(guide.faqs || [])
      }
    } catch (error) {
