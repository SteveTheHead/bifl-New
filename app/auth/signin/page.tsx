...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use simple authentication for admin login
      ...
      if (response.ok && data.success) {
        // Show success message with link to admin
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        {!success ? (
          ...
          <div className="mt-8 text-green-600 text-center bg-green-50 border border-green-200 rounded-lg p-6">
            <div className="font-medium mb-2 text-lg">Authentication Successful!</div>
            <div className="mb-4">You have been signed in as admin.</div>
            <div className="mb-4 text-sm text-gray-600">
              Please manually navigate to: <strong>http://localhost:3000/admin</strong>
            </div>
            <div className="mb-4">
              <a
                href="/admin"
                className="inline-block bg-brand-teal text-white px-6 py-3 rounded-lg hover:opacity-90 transition-opacity font-medium text-decoration-none"
            ...
            <div className="text-xs text-gray-500">
              If the link above doesn't work, copy and paste the URL above into your address bar.
            </div>
          </div>
        )}
      </div>
